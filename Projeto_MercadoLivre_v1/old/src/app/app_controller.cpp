...
int AppController::run() {
    ...
    while (true) {
        ...
        std::cout << "3. Verificar parser (DEBUG)\n";  // Nova opção
        std::cout << "0. Sair\n";
        ...
        switch (option) {
            ...
            case 2:
                ...
                break;
            case 3:
                executeParserVerification();  // Novo método
                break;
            default:
...
bool AppController::executeParserVerification() {
    ...
    // Pedir ao usuário para selecionar uma instância
    int selection;
    std::cout << "Selecione o número da instância para verificar (0 para voltar): ";
    std::cin >> selection;
    std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
    ...
    try {
        // Ler a instância selecionada
        InputParser parser;
        Warehouse warehouse = parser.parseFile(selectedInstance);
        ...
        for (int p = 0; p < warehouse.numOrders; p++) {
            // Mostrar apenas os primeiros 5 pedidos para não sobrecarregar a tela
            if (p > 4 && warehouse.numOrders > 10) {
                std::cout << "...(apenas os 5 primeiros pedidos mostrados de " 
                          << warehouse.numOrders << " totais)..." << std::endl;
                break;
            }
        ...
        for (int c = 0; c < warehouse.numCorridors; c++) {
            // Mostrar apenas os primeiros 5 corredores para não sobrecarregar a tela
            if (c > 4 && warehouse.numCorridors > 10) {
                std::cout << "...(apenas os 5 primeiros corredores mostrados de " 
                          << warehouse.numCorridors << " totais)..." << std::endl;
                break;
            }
