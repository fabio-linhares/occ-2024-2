...
struct AuxiliaryStructures {
    ...
    struct ItemInfo {
        ...
        int disponibilidade_atual = 0;           // NOVO: para controle dinâmico de estoque
        double escassez = 0.0;                     
...
// Função para filtrar pedidos viáveis (todos os itens disponíveis em quantidade suficiente)
inline std::vector<int> filtrarPedidosViaveis(const AuxiliaryStructures& aux, const Warehouse& warehouse) {
    std::vector<int> pedidos_viaveis;
    ...
    for (const auto& pedido : aux.pedidos_aprimorado) {
        ...
        if (viavel) {
            pedidos_viaveis.push_back(pedido.id);
        }
    }
...
// Função para reservar itens de um pedido (controle de estoque)
inline bool reservarItens(AuxiliaryStructures& aux, int pedido_id) {
    const auto& pedido = aux.pedidos_aprimorado[pedido_id];
    
    // Verificar disponibilidade atual
    for (const auto& [item_id, quantidade] : pedido.itens) {
        if (item_id >= aux.itens_aprimorado.size() || 
            aux.itens_aprimorado[item_id].disponibilidade_atual < quantidade) {
            return false; // Não há disponibilidade suficiente
        }
    }
    
    // Reservar os itens (decrementar disponibilidade)
    for (const auto& [item_id, quantidade] : pedido.itens) {
        aux.itens_aprimorado[item_id].disponibilidade_atual -= quantidade;
    }
...
// Função para liberar itens de um pedido
inline void liberarItens(AuxiliaryStructures& aux, int pedido_id) {
    const auto& pedido = aux.pedidos_aprimorado[pedido_id];
    
    // Liberar os itens (incrementar disponibilidade)
    for (const auto& [item_id, quantidade] : pedido.itens) {
        if (item_id < aux.itens_aprimorado.size()) {
            aux.itens_aprimorado[item_id].disponibilidade_atual += quantidade;
        }
    ...
    for (int c = 0; c < warehouse.numCorridors; c++) {
        ...
        for (const auto& item_pair : warehouse.corridors[c]) {
            ...
            if (item_id < warehouse.numItems) {
                ...
                aux.itens_aprimorado[item_id].disponibilidade_total += quantidade;
                aux.itens_aprimorado[item_id].disponibilidade_atual += quantidade; // NOVO
            }
        }
...
inline void calcularPrioridadePedidos(AuxiliaryStructures& aux, 
    ...
    for (const auto& pedido : aux.pedidos_aprimorado) {
        ...
        // Verificar se atende aos limites LB/UB 
        // Observação: aqui estamos verificando apenas o UB; o LB será garantido pelo algoritmo de seleção
        bool dentro_limite_superior = pedido.total_itens <= warehouse.UB;
        if (!dentro_limite_superior) {
            continue; // Pular pedido que excede o limite superior
        }
        
        // Calcular fator de raridade
        double fator_raridade = 0.0;
        bool todos_itens_disponiveis = true;
        ...
        for (const auto& item_pair : pedido.itens) {
            ...
            if (item_id >= aux.itens_aprimorado.size() || 
                aux.itens_aprimorado[item_id].disponibilidade_total < quantidade) {
                todos_itens_disponiveis = false;
                break;
            }
            
            // Calcular componente de raridade
            if (item_id < aux.itens_aprimorado.size()) {
                fator_raridade += aux.itens_aprimorado[item_id].escassez * quantidade;
            }
        }
        
        // Pular pedidos com itens indisponíveis
        if (!todos_itens_disponiveis) {
            continue;
        }
        
        // Normalizar fator de raridade
        if (!pedido.itens.empty()) {
            fator_raridade /= pedido.itens.size();
        }
        
        // Calcular eficiência (itens/corredores)
        double eficiencia = 0.0;
        if (!pedido.corredores_necessarios.empty()) {
            eficiencia = static_cast<double>(pedido.total_itens) / 
                        pedido.corredores_necessarios.size();
        }
        
        // Prioridade combinando eficiência e raridade
        double prioridade = eficiencia * (1.0 + 0.2 * fator_raridade);
    ...
    std::cout << "    Priorização de pedidos concluída. " 
              << pedidos_priorizados.size() << " pedidos viáveis encontrados." << std::endl;
}

// Nova função para gerar uma solução inicial válida
inline bool gerarSolucaoValidaInicial(const Warehouse& warehouse, Solution& solution) {
    // Limpar solução existente
    solution.clear();
    
    std::cout << "=== Gerando solução inicial válida ===" << std::endl;
    std::cout << "Limites: LB=" << warehouse.LB << ", UB=" << warehouse.UB << std::endl;
    
    // Inicializar estruturas auxiliares
    AuxiliaryStructures aux;
    cria_auxiliares(warehouse, solution);
    
    // Inicializar estruturas aprimoradas
    inicializarEstruturasAprimoradas(aux, warehouse);
    calcularMetricasAvancadas(aux);
    
    // Para cada item, inicializar disponibilidade_atual = disponibilidade_total
    for (auto& item : aux.itens_aprimorado) {
        item.disponibilidade_atual = item.disponibilidade_total;
    }
    
    // Calcular prioridades e obter pedidos ordenados
    std::vector<std::pair<int, double>> pedidos_priorizados;
    calcularPrioridadePedidos(aux, warehouse, pedidos_priorizados);
    
    // Variáveis para rastrear itens e corredores selecionados
    int total_itens = 0;
    std::set<int> corredores_visitados;
    
    // Selecionar pedidos até atingir LB, respeitando UB
    for (const auto& [pedido_id, prioridade] : pedidos_priorizados) {
        const auto& pedido = aux.pedidos_aprimorado[pedido_id];
        
        // Verificar se adicionar esse pedido excederia UB
        if (total_itens + pedido.total_itens > warehouse.UB) {
            continue;
        }
        
        // Verificar/reservar disponibilidade de itens
        if (!reservarItens(aux, pedido_id)) {
            continue; // Itens não disponíveis em quantidade suficiente
        }
        
        // Adicionar pedido à solução
        solution.addOrder(pedido_id, warehouse);
        ...
        for (int corredor_id : pedido.corredores_necessarios) {
            ...
            solution.addVisitedCorridor(corredor_id);
        }
        
        std::cout << "Adicionado pedido #" << pedido_id << " com " << pedido.total_itens 
                  << " itens. Total atual: " << total_itens << "/" << warehouse.LB << std::endl;
    ...
    // Verificar se a solução atende LB
    if (total_itens < warehouse.LB) {
        std::cout << "ERRO: Não foi possível atingir o limite inferior (" << warehouse.LB 
                  << "). Total conseguido: " << total_itens << std::endl;
        return false;
    }
    
    // Calcular função objetivo
    double objetivo = static_cast<double>(total_itens) / corredores_visitados.size();
    
    // Mostrar resultado
    std::cout << "\n=== Solução inicial válida gerada ===\n";
    std::cout << "- Total de itens: " << total_itens << "\n";
    std::cout << "- Pedidos selecionados: " << solution.getSelectedOrders().size() << "\n";
    std::cout << "- Corredores visitados: " << corredores_visitados.size() << "\n";
    std::cout << "- Função objetivo (itens/corredores): " << objetivo << std::endl;
