...
Solucao solucionadorRobusto(const Problema& problema) {
    ...
    // Abordagem 3: ILS
    Solucao solucao3 = aplicarILS(problema, params, metricas);
    
    // Se a instância for pequena o suficiente, tentar busca exaustiva
    Solucao solucao4;
    if (problema.o <= 8) {
        solucao4 = buscaExaustiva(problema);
    } else {
        solucao4.custo_total = std::numeric_limits<double>::max();
    }
    
    // Retornar a melhor solução entre as quatro abordagens
    if (solucao1.custo_total <= solucao2.custo_total && solucao1.custo_total <= solucao3.custo_total && solucao1.custo_total <= solucao4.custo_total) {
        return solucao1;
    } else if (solucao2.custo_total <= solucao1.custo_total && solucao2.custo_total <= solucao3.custo_total && solucao2.custo_total <= solucao4.custo_total) {
        return solucao2;
    } else if (solucao3.custo_total <= solucao1.custo_total && solucao3.custo_total <= solucao2.custo_total && solucao3.custo_total <= solucao4.custo_total) {
        return solucao3;
    } else {
        return solucao4;
    }
