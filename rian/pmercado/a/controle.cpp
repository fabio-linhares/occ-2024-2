...
void registrarDesempenho(const std::vector<ResultadoInstancia>& resultados, const std::vector<Problema>& problemas) {
    ...
    for (const auto& resultado : resultados) {
        tempo_total_ms += resultado.tempo_execucao_ms;
    }
    
    for (size_t i = 0; i < resultados.size(); ++i) {
        MetricasDesempenho metrica;
        metrica.nome_instancia = resultados[i].nome_instancia;
        metrica.num_pedidos_atendidos = resultados[i].solucao.pedidos_atendidos.size();
        metrica.num_corredores_utilizados = resultados[i].solucao.corredores_utilizados.size();
        metrica.total_itens = calcularTotalItens(resultados[i].solucao, problemas[i]);
        metrica.razao_itens_corredor = resultados[i].solucao.custo_total;
        metrica.tempo_execucao_ms = resultados[i].tempo_execucao_ms;
        metrica.algoritmo_utilizado = "Adaptativo";
        metrica.data_execucao = data;
        metrica.tempo_total_ms = tempo_total_ms; // Adicionando tempo total
...
void exibirEstatisticasTerminal(const std::vector<ResultadoInstancia>& resultados, const std::vector<Problema>& problemas) {
    ...
    std::cout << "Data: " << data << "\n\n";
    std::cout << "TEMPO TOTAL DE EXECUÇÃO: " << tempo_total_ms << " ms\n\n";
...
void salvarHistoricoDesempenho(const std::vector<MetricasDesempenho>& metricas) {
    ...
    if (!arquivo_existe) {
        out << "Data,Instância,Pedidos,Corredores,Itens,Razão,Tempo(ms),Algoritmo,TempoTotal(ms)\n";
    }
